...
import {
  clinicDetailPublic,
  patientQrRegister,
} from "../../../services/apis/patient.service";
    ...
    async (e) => {
      const validate = () => {
        ...
        if (!form.complaints.trim())
          errs.complaints = "Please describe your complaints";
      ...
      <div className={styles.PatientAppointment_container}>
        {stage === "loading" && (
          <div className={styles.PatientAppointment_loadingScreen}>
            ...
            <p className={styles.PatientAppointment_loadingText}>
          ...
          <div className={styles.PatientAppointment_noClinicScreen}>
            <header className={styles.PatientAppointment_header}>
              <img
                src={gridVitalLogo}
                alt="GridVital"
                className={styles.PatientAppointment_logo}
              />
            </header>
            ...
            <div className={styles.PatientAppointment_noClinicIconWrap}>
              <QrCode
                size={42}
                className={styles.PatientAppointment_noClinicIcon}
              />
            </div>

            <h3 className={styles.PatientAppointment_noClinicTitle}>
              Invalid QR Code
            </h3>

            <p className={styles.PatientAppointment_noClinicMessage}>
              The appointment link is missing clinic information. Please scan
              the QR code again or contact the clinic to get a valid link.
            </p>
        ...
        {stage === "form" && (
          <>
            ...
            <div className={styles.PatientAppointment_titleCard}>
              <h2 className={styles.PatientAppointment_title}>
            ...
            <div className={styles.PatientAppointment_clinicCard}>
              <div className={styles.PatientAppointment_clinicInfo}>
                {clinicName && (
                  <h3 className={styles.PatientAppointment_clinicName}>
                    {clinicName}
                  </h3>
                )}
                {(doctorName || specialization) && (
                  <p className={styles.PatientAppointment_doctorRow}>
                    {doctorName && (
                      <span className={styles.PatientAppointment_doctorName}>
                        {doctorName}
                      </span>
                    )}
                    {doctorName && specialization && (
                    ...
                    {specialization && (
                      <span
                        className={styles.PatientAppointment_specialization}
                      >
                        {specialization}
                      </span>
                    )}
            ...
            <form
              className={styles.PatientAppointment_form}
              onSubmit={handleSubmit}
              noValidate
            >
              <div className={styles.PatientAppointment_field}>
                <label className={styles.PatientAppointment_label}>
                  Full Name
                </label>
                <input
                ...
                />
                {errors.name && (
                  <span className={styles.PatientAppointment_error}>
                    {errors.name}
                  </span>
                )}
              </div>
            ...
            {modalOpen && (
              ...
              >
                <div className={styles.PatientAppointment_modalBox}>
                  <h3 className={styles.PatientAppointment_modalTitle}>
                    Data Privacy & Digital Token Consent
                  </h3>
                  <div className={styles.PatientAppointment_modalBody}>
                    <h4 className={styles.PatientAppointment_consentSectionTitle}>
                      Data Privacy & Digital Token Consent (डेटा गोपनीयता एवं टोकन सहमति)
                    </h4>
                    <p>
                      By checking this box, you explicitly agree that the name, mobile number, age, and health complaints provided in this form will be securely processed and shared with this registered clinic for the sole purpose of queue tracking, token generation, and medical record-keeping. Your data is isolated under encrypted protocols and will never be shared with third-party pharmaceutical or marketing companies without your explicit authorization.
                    </p>
                    <p>
                      इस बॉक्स को टिक करके, आप अपनी सहमति देते हैं कि इस फॉर्म में भरा गया आपका नाम, मोबाइल नंबर, उम्र और बीमारी के लक्षण इस क्लीनिक के साथ सुरक्षित रूप से साझा किए जाएंगे, ताकि आपका लाइव टोकन नंबर जनरेट किया जा सके। आपका डेटा पूरी तरह सुरक्षित है और इसे आपकी अनुमति के बिना किसी भी बाहरी या फार्मा कंपनी को नहीं बेचा जाएगा।
                    </p>

                    <h4 className={styles.PatientAppointment_consentSectionTitle}>
                      ⚠️ IMPORTANT RESPONSIBILITY DISCLAIMER (महत्वपूर्ण कानूनी चेतावनी)
                    </h4>
                    <p className={styles.PatientAppointment_modalWarning}>
                      <strong>
                        GridVital functions strictly as a technological token routing and queue scheduling infrastructure. GridVital does not provide medical consultation, diagnosis, or clinical advice. The absolute responsibility for your medical checkup, diagnostic analysis, treatment quality, and the final written prescription lies solely and exclusively with the practicing Doctor/Clinic.
                      </strong>
                    </p>
                    <p className={styles.PatientAppointment_modalWarning}>
                      <strong>
                        GridVital केवल एक डिजिटल टोकन और कतार प्रबंधन (Queue Management) सॉफ्टवेयर है। यह कोई चिकित्सीय सलाह या डायग्नोसिस नहीं देता है। आपके इलाज, डॉक्टर द्वारा की जाने वाली जांच, और पर्चे (Prescription) पर लिखी दवाओं की पूरी कानूनी और व्यावहारिक जिम्मेदारी सिर्फ और सिर्फ डॉक्टर/क्लीनिक की होगी।
                      </strong>
                    </p>
                  </div>
